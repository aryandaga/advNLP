"""
QLoRA Fine-Tuning on Llama-2-7B via Modal
==========================================
Runs on Modal A10G GPU (24GB VRAM).
- Base model: meta-llama/Llama-2-7b-hf (4-bit NF4 quantized)
...
# ── Modal app setup ────────────────────────────────────────────────────────────
app = modal.App("llama-qlora-bias")
...
def run_qlora():
    ...
    # ── Baseline eval (4-bit, no adapter) ────────────────────────────────────
    print("\nLoading baseline Llama-2-7B (4-bit NF4)...")
    ...
    for row in tqdm(crows, desc="  DirectBias baseline", leave=False):
    ...
    args = TrainingArguments(
        ...
        learning_rate=2e-4,
        fp16=False,
        bf16=False,        # bfloat16 off for 4-bit training stability
        logging_steps=50,
        save_strategy="no",
        report_to="none",
    ...
    # ── Post-QLoRA eval (keep as PeftModel — can't merge 4-bit) ──────────────
    print("\nEvaluating post-QLoRA model...")
    model.eval()

    qlora_sst2 = sst2_accuracy(model)
    print(f"Post-QLoRA SST-2 accuracy: {qlora_sst2:.3f}")

    qlora_sps = crows_sps(model)
    print(f"Post-QLoRA SPS: {qlora_sps:.1f}%")

    g_qlora = compute_gender_dir(model, tokenizer)
    stereo_vecs, anti_vecs = [], []
    for row in tqdm(crows, desc="  DirectBias QLoRA", leave=False):
        dm, dl = get_diff_words(row["sent_more"], row["sent_less"])
        for w in dm: stereo_vecs.append(get_repr(model, tokenizer, w))
        for w in dl: anti_vecs.append(get_repr(model, tokenizer, w))
    qlora_db_stereo = direct_bias(stereo_vecs, g_qlora)
    qlora_db_anti   = direct_bias(anti_vecs,   g_qlora)
    print(f"Post-QLoRA DirectBias stereo={qlora_db_stereo:.4f} anti={qlora_db_anti:.4f}")

    # ── Save results ──────────────────────────────────────────────────────────
    results = {
        "model": MODEL_NAME,
        "method": "QLoRA",
        "lora_config": {"r": 8, "alpha": 16, "target": "q_proj,v_proj", "quant": "4bit-nf4"},
        "baseline": {
        ...
        "post_qlora": {
            "sst2_accuracy": round(qlora_sst2, 4),
            "crows_sps": round(qlora_sps, 2),
            "direct_bias_stereo": round(qlora_db_stereo, 4),
            "direct_bias_anti": round(qlora_db_anti, 4),
        },
    }

    out_path = os.path.join(VOLUME_PATH, "llama_qlora_results.json")
